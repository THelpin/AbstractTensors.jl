...
### 3 — `is_canonical_less` (production: `tensor_id`)

**Workload:** 4000 comparisons (250 repeats × 4×4 component pairs) using production [`is_canonical_less`](@ref): [`tensor_id`](@ref) on tensor heads, then lexicographic indices.

**Benchmark 3b** (same loop) keeps the legacy `print_as` string compare for A/B reference.

**Typical scale:** `tensor_id` Int compare should match or beat the old ~18 μs print_as baseline with **0 allocations**.
...
This is what **not** to ship: Dict lookup dominates cost. On a reference run, production `tensor_id` was ~18 μs vs Dict ~324 μs — **do not** use a `Dict` for canonical sort keys.
...
- `a.registry_id < b.registry_id` — stand-in for `tensor_id(a.tensor) < tensor_id(b.tensor)`
...
After `tensor_id` on [`Tensor`](@ref), benchmark 3 is the production path; benchmark 5 remains a mock sanity check.

**How to read results:**

- If benchmark 5 field ≈ or faster than benchmark 5 label → Int head ids beat string labels (production uses `tensor_id`).
...
Benchmark 3 is production `is_canonical_less`; benchmark 3b is the legacy `print_as` baseline.
...
For A/B tests (e.g. before/after `tensor_id`):
