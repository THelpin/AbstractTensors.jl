...
Root abstract type for all vector bundle singleton types.
Concrete subtypes are generated by [`@def_manifold`](@ref):
one [`TangentBundle`](@ref) subtype and one [`CoTangentBundle`](@ref)
subtype per manifold.
"""
abstract type VBundle end
...
Abstract supertype for tangent bundle singletons.
Every concrete subtype is generated by [`@def_manifold`](@ref) and
carries `isdual = false` in its [`VBundleRecord`](@ref).
Indices in a tangent bundle are contravariant (upper).
"""
abstract type TangentBundle   <: VBundle end
...
Abstract supertype for cotangent (dual) bundle singletons.
Every concrete subtype is generated by [`@def_manifold`](@ref) and
carries `isdual = true` in its [`VBundleRecord`](@ref).
Indices in a cotangent bundle are covariant (lower).
"""
abstract type CoTangentBundle <: VBundle end
...
Return `true` if `x` is a subtype of [`Manifold`](@ref), `false` otherwise.

    is_manifold(M)        # true  (after @def_manifold M ...)
    is_manifold(Float64)  # false
"""
...
Return `true` if `x` is a subtype of [`VBundle`](@ref), `false` otherwise.
"""
is_vbundle(::Type{<:VBundle}) = true
is_vbundle(::Any)              = false

"""
    is_tangent_bundle(x) -> Bool

Return `true` if `x` is a subtype of [`TangentBundle`](@ref), or if `x` is a
`Symbol` key of a non-dual bundle in `_VBUNDLES`. `false` otherwise.
"""
is_tangent_bundle(::Type{<:TangentBundle})     = true
is_tangent_bundle(::Any)                       = false

"""
    is_cotangent_bundle(x) -> Bool

Return `true` if `x` is a subtype of [`CoTangentBundle`](@ref), or if `x` is a
`Symbol` key of a dual bundle in `_VBUNDLES`. `false` otherwise.
"""
...
"""
    base_manifold(VB::Type{<:VBundle}) -> Type{<:Manifold}

Return the base manifold type for bundle `VB`.
Concrete methods are generated by [`@def_manifold`](@ref) for every
tangent and cotangent bundle it creates.
Errors for any type that is not a registered bundle.
"""
...
"""
    manifold_info(M::Type{<:Manifold})

Print a human-readable summary of manifold `M`: dimension, tangent and
cotangent bundle names, all associated vector bundles, and index symbols.
"""
